...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  const hasChunks = !isUser && !message.isLoading && message.chunks && message.chunks.length > 0
  ...
  return (
    <motion.div
      ...
      className={cn('flex gap-3 px-4 md:px-8 py-3', isUser ? 'flex-row-reverse' : 'flex-row')}
    >
      {/* Avatar */}
      <div
        className={cn(
      ...
      <div className={cn('flex flex-col gap-1.5 max-w-[80%]', isUser ? 'items-end' : 'items-start')}>
        ...
        {hasChunks && (
          <div className="w-full mt-1 space-y-2">
            ...
            >
              {chunksExpanded ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
              <span>
                {chunksExpanded ? 'Ocultar' : 'Ver'} {message.chunks!.length}{' '}
                {message.chunks!.length === 1 ? 'trecho fonte' : 'trechos fonte'}
              </span>
            </button>
            ...
            {chunksExpanded && (
              ...
              >
                {message.chunks!.map((chunk, i) => (
                  <SourceChunk key={`${chunk.filename}-${chunk.chunk_index}`} chunk={chunk} index={i} />
                ))}
              </motion.div>
            )}
          </div>
